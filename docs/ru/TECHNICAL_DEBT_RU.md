...
**[Сводка реализации улучшений CI/CD](CI_CD_IMPROVEMENTS_RU.md)** — Подробная сводка реализованных оптимизаций CI/CD.

**[Эволюция Testcontainers и стратегия](TESTCONTAINERS_EVOLUTION_RU.md)** — Подробное руководство по использованию Testcontainers, эволюции и когда его реализовывать.
...
- **Оптимизация CI/CD**: Реализованы финальные рекомендации для стабильного CI/CD:
  - Полный переход на MySQL во всех средах (включая unit тесты)
  - Добавлены явные ENV переменные в GitHub Actions
  - Интегрирована валидация Flyway миграций
  - Добавлено логирование активного профиля Spring
  - Настроено автоматическое создание тестовой БД в CI
  - Удалена H2 зависимость для обеспечения production-ready тестирования
...
- **Файловое хранилище (например, S3, MinIO)**
  - **Назначение**: Для хранения загружаемого пользователями контента, такого как изображения и документы.
    Решение будет принято на основе финальных требований к функциональности.
