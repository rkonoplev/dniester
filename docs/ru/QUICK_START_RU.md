...
docker compose up
```
— Docker Compose запускает все контейнеры и “прицепляет” их логи прямо к вашему терминалу. Вы увидите бегущие логи `phoebe-app`, `phoebe-mysql`, `phoebe-nextjs` и так далее.
...
- **Утром**: Убедитесь, что Docker Desktop (или Docker Engine) запущен. Перейдите в папку проекта и запустите проект:
  ```bash
  ...
  docker compose up
  ```
  ⚙️ Всё поднимется в том же состоянии, база данных сохранится, так как volume `mysql_data` не удаляется.
