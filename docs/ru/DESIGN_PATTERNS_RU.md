...
-   **Singleton (Одиночка)**
    -   **Реализация:** Применяется фреймворком Spring ко всем компонентам, управляемым через IoC-контейнер
        (аннотированным `@Service`, `@Repository`, `@Controller`, `@Component`).
    -   **Назначение:** Spring по умолчанию создает только один экземпляр (бин) каждого компонента, который
        переиспользуется во всем приложении. Это обеспечивает консистентность состояния и экономию ресурсов.
        Однако, в контексте данных, паттерн Одиночка также может быть применен к сущностям,
        которые представляют уникальную, общесистемную конфигурацию.
    -   **Пример (Одиночка данных):** Сущность `ChannelSettings` предназначена для хранения глобальных,
        ...
        ее Одиночкой на уровне данных. Сервисный слой приложения обеспечивает эту уникальность, всегда
...
-   **Builder (Строитель)**
    ...
    -   **Назначение:** Позволяет конструировать объекты пошагово, делая код более читаемым по сравнению
        с конструкторами с большим количеством параметров.
...
-   **Adapter (Адаптер)**
    -   **Реализация:** Частично реализуется через использование DTO (Data Transfer Object).
    -   **Назначение:** DTO адаптируют внутреннюю структуру данных (JPA-сущности) к формату, необходимому
        для внешних клиентов (API-ответы), скрывая внутренние детали и предотвращая их утечку.
...
-   **Strategy (Стратегия)**
    -   **Реализация:** Является основой многих компонентов Spring, особенно Spring Security.
...
-   **Dependency Injection (DI) / Inversion of Control (IoC)**
    ...
    -   **НазнаGчение:** Компоненты не создают свои зависимости, а получают их извне от IoC-контейнера.
