...
1.  **Сборка и запуск контейнеров:** `docker-compose` соберет образ вашего Spring Boot приложения и запустит
    два контейнера: `news-app` (приложение) и `news-mysql` (база данных MySQL 8.0).

2.  **Создание пустой базы:** При первом запуске контейнер `news-mysql` создаст пустую базу данных с именем `dniester`.

3.  **Автоматические миграции Flyway:**
    - Ваше Spring Boot приложение (`news-app`) стартует и подключится к пустой базе.
    - **Flyway**, встроенный в приложение, обнаружит, что база пуста.
    - Flyway последовательно выполнит все SQL-скрипты из папки `backend/src/main/resources/db/migration/common`:
      ...
      - `V3__insert_sample_data.sql`: **Заполнит эти таблицы данными**, включая пользователей, роли и контент,
        который был сконвертирован из оригинального дампа Drupal 6.
      - Последующие скрипты (`V4`, `V5` и т.д.) применят все дальнейшие изменения в схеме.
...
USE dniester;
SELECT COUNT(*) FROM content; -- Должно показать ~12186 записей
SELECT COUNT(*) FROM users;   -- Должно показать ~400+ пользователей
```
