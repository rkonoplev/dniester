...
5.  **Сканирование на секреты (GitLeaks)**
    - Хотя GitLeaks больше не является частью автоматического CI-пайплайна, рекомендуется использовать его локально перед отправкой изменений, чтобы избежать случайной утечки ключей и паролей.
      ```bash
