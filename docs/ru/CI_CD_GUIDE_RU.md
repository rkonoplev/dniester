...
2.  **Сборка (Build)**
    ...
    - Spring использует `application-ci.yml` → MySQL через Docker Compose (продакшн-подобная среда).
...
3.  **Профили для CI**
    - Всегда запускайте тесты в CI с профилем `ci` (использует MySQL через Docker Compose).
    - Это гарантирует тестирование против продакшн-подобной среды базы данных.

6.  **Стратегия тестирования базы данных**
    - **Unit тесты**: Используют моки, без зависимостей от базы данных
    - **Локальные интеграционные тесты**: Используют Testcontainers с MySQL (`LocalIntegrationTest`)
    - **CI интеграционные тесты**: Используют внешний MySQL из Docker Compose (`AbstractIntegrationTest`)
    - **Выбор профиля**: Автоматический на основе среды (`ci-integration` для CI, `integration-test` для локальной)
...
7.  **Стратегия базы данных**
    ...
    - **Гибридное тестирование**: Testcontainers для локальной разработки, Docker Compose для CI
    - **Оптимизация среды**: CI использует общий сервис MySQL для более быстрого выполнения
