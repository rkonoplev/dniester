...
Для первого запуска проекта, пожалуйста, обратитесь к **[Руководству по первоначальной настройке проекта](./SETUP_GUIDE_RU.md)**.
...
- **Запуск полного окружения** (БД + бэкенд):
  ```bash
  make run
  ```
...
- **Запуск приложения вручную**:
  1. Поднимите базу данных: `docker-compose up -d phoebe-mysql`
  2. Запустите приложение: `cd backend && ./gradlew bootRun`
...
> **Примечание**: Сканирование на секреты (GitLeaks) больше не является автоматическим шагом в CI и рекомендуется для локального запуска.
