...
*   **Важно**: Бэкенд **не разрешает** встраивание тегов `<img>` и `<iframe>` в поля контента из соображений безопасности (XSS). Фронтенд должен либо не предоставлять такую функциональность, либо предупреждать пользователя о том, что эти теги будут удалены при сохранении.
...
- **Разрешенный контент**: Только форматирование текста и вставка гиперссылок.
...
*   **Примечание**: Теги `<img>` и `<iframe>` не входят в белый список разрешенных тегов.
...
- **Управление паролями**: Пароли хранятся в базе данных в хешированном виде.
