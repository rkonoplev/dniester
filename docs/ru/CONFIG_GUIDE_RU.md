...
|:-------------------|:-------------------------------------|:-------------------|:-------------------|:----------------------------------------------------------------------------|
| `dev`              | **Актуальный**                       | `main/resources`   | MySQL (Docker)     | Разработка с использованием Docker Compose (`make run`). **Основной профиль для разработки.** |
| `local`            | **Актуальный**                       | `main/resources`   | MySQL (Локальная)  | Локальная разработка без Docker (например, запуск из IDE).                  |
...
| `ci`               | **Устарел**                          | `legacy`           | H2 (в памяти)      | Ранее использовался в CI. **Больше не используется.**                       |
...
**Актуальные способы:**
```bash
# Запуск для разработки с Docker (активирует профиль 'dev')
make run
