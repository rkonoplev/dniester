...
Пути к миграциям Flyway настраиваются через профили Spring, что позволяет комбинировать общие и специфичные
для БД скрипты.

| Миграция | Назначение | Изменения | Расположение |
|-----------|------------|-----------|--------------|
| V1 | Начальная схема | Основные таблицы: users, roles, content, terms | `common` |
| V2 | Рабочий процесс публикации | Добавлена колонка `published` в content | `common` |
...
| V6 | Настройка разрешений | Заполнены разрешения по умолчанию и назначения ролей | `mysql`, `postgresql` |
| V7 | Добавление уникальных ограничений | Добавлены уникальные ограничения и индексы в join-таблицы | `mysql`, `postgresql` |
