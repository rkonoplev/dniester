...
2.  **Доступ через API приложения:**
    *   Все операции с данными (чтение, запись, обновление, удаление) должны выполняться через API вашего приложения. Это обеспечивает соблюдение бизнес-логики, валидации и механизмов безопасности.
