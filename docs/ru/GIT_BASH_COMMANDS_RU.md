...
- [Откат изменений и восстановление](#откат-изменений-и-восстановление)
- [Управление зависшими Testcontainers и Docker](#управление-зависшими-testcontainers-и-docker)
...
### Полезные цепочки команд
```bash
# Проверить файл, сделать исполняемым и запустить тесты
ls -la gradlew && chmod +x gradlew && ./gradlew integrationTest

# Разбор команды:
# - `ls -la gradlew` - проверяет существование файла gradlew и показывает его права доступа
# - `&&` - логический оператор "И" (выполняет следующую команду только если предыдущая успешна)
# - `chmod +x gradlew` - делает файл gradlew исполняемым (добавляет права на выполнение)
# - `./gradlew integrationTest` - запускает интеграционные тесты через Gradle Wrapper

# Зачем это нужно:
# - Иногда после клонирования репозитория файл gradlew теряет права на выполнение
# - Команда автоматически проверяет и исправляет это
# - Если любой шаг не удался, выполнение останавливается

# Другие полезные цепочки команд
# Остановить контейнеры, очистить и запустить заново
docker compose down && docker system prune -f && docker compose up -d

# Проверить статус и подключиться к базе
docker ps && docker exec -it phoebe-mysql mysql -uroot -proot

# Собрать приложение и запустить все тесты
