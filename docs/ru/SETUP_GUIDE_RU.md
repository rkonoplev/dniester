...
В корневой директории проекта находится файл `docker-compose.yml`. Он ключевой для локальной разработки,
оркестрируя несколько сервисов в Docker-контейнерах. Файл определяет и запускает:

-   **`phoebe-mysql`**: Контейнер с базой данных MySQL.
-   **`phoebe-app`**: Контейнер с бэкенд-приложением Spring Boot.
-   **`nextjs-app`**: Контейнер с референсным фронтенд-приложением на Next.js.

Использование `docker-compose.yml` позволяет быстро поднять все компоненты проекта одной командой,
обеспечивая согласованное окружение для разработки.
...
2.  **Запустите все одной командой**:
    ...
    make run
    ```
    Эта команда запустит **все необходимые сервисы**: базу данных (`phoebe-mysql`), бэкенд
    (`phoebe-app`) и референсный фронтенд на Next.js (`nextjs-app`).
    *Альтернативная команда (без Makefile):* `docker compose up --build`

**Результат**: `docker-compose` поднимет приложение и базу данных. Flyway автоматически создаст схему
и наполнит ее всеми данными из миграционных скриптов. Этот процесс подробно описан в
**[Актуальном руководстве по миграции](./MODERN_MIGRATION_GUIDE_RU.md)**.
...
Это позволит изолировать проблему и проверить взаимодействие с бэкендом.

> Для более детальной информации по запуску и устранению неполадок Next.js фронтенда,
> включая специфические проблемы с Docker, обратитесь к разделу
> **[Устранение неполадок в Технической спецификации Frontend (Next.js)](./FRONTEND_SPEC_NEXTJS_RU.md#устранение-неполадок)**.

### 1. Убедитесь, что бэкенд запущен

Прежде чем запускать фронтенд, убедитесь, что бэкенд работает и доступен по адресу
`http://localhost:8080`. Вы можете запустить его, используя `make run` (как в Сценарии 2)
или `cd backend && ./gradlew bootRun`.

### 2. Запустите выбранный фронтенд вручную

Проект Phoebe CMS предоставляет два референсных фронтенда: на Angular и на Next.js.
Выберите тот, с которым вы хотите работать.

#### Для Angular фронтенда:

1.  Перейдите в директорию Angular фронтенда:
    ```bash
    cd frontends/angular
    ```
2.  Установите зависимости (если вы это не делали ранее):
    ```bash
    npm install
    ```
3.  Запустите Angular приложение:
    ...
    npm start
    ```
    Приложение будет доступно по адресу `http://localhost:4200` (или другому,
    указанному в консоли).

#### Для Next.js фронтенда:

1.  Перейдите в директорию Next.js фронтенда:
    ```bash
    cd frontends/nextjs
    ```
2.  Установите зависимости (если вы это не делали ранее):
...
3.  Запустите Next.js приложение:
    ...
    npm run dev
    ```
    Приложение будет доступно по адресу `http://localhost:3000` (или другому,
    указанному в консоли).

### 3. Проверка логов фронтенда

Если фронтенд не запускается или работает некорректно, проверьте его логи:

-   **Для Next.js (если запущен через `make run` или `docker compose up`):**
    ```bash
    docker compose logs -f nextjs-app
    ```
-   **Для Angular или Next.js (если запущены вручную):**
    Логи будут отображаться непосредственно в терминале, где вы запускали
