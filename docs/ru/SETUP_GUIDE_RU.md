...
Для полноценной работы с проектом вам необходимо запустить не только бэкенд (API), но и один из референсных фронтендов. Фронтенд будет взаимодействовать с бэкендом по API, поэтому ему неважно, как именно запущен бэкенд (в Docker или напрямую), и какая база данных используется (Dockerized MySQL/PostgreSQL или локально установленная).

### 1. Убедитесь, что бэкенд запущен

Прежде чем запускать фронтенд, убедитесь, что бэкенд работает и доступен по адресу `http://localhost:8080`. Вы можете запустить его, используя `make run` (как в Сценарии 2) или `cd backend && ./gradlew bootRun`.

### 2. Запустите выбранный фронтенд

Проект Phoebe CMS предоставляет два референсных фронтенда: на Angular и на Next.js. Выберите тот, с которым вы хотите работать.
...
3.  Запустите Angular приложение:
    ...
    npm start
    ```
    Приложение будет доступно по адресу `http://localhost:4200` (или другому, указанному в консоли).

#### Для Next.js фронтенда:

11. Перейдите в директорию Next.js фронтенда:
    ```bash
    cd frontends/nextjs
    ```
12. Установите зависимости (если вы это не делали ранее):
    ```bash
    npm install
    ```
13. Запустите Next.js приложение:
    ```bash
    npm run dev
    ```
    Приложение будет доступно по адресу `http://localhost:3000` (или другому, указанному в консоли).
